...
*/
   ...
   if (!strcmp(theConfig.scheduling, "FIFO")){
      for (ndx = 0; ndx < numOfProcesses; ++ndx){
         runProcess(ndx);
      }
   }
...
void addTime(){
   if (tvalAfter.tv_usec > tvalBefore.tv_usec){
      int newSecs = tvalAfter.tv_sec - tvalBefore.tv_sec;
      double calc = (double)(tvalAfter.tv_sec - tvalBefore.tv_sec)
                                    + (double)(tvalAfter.tv_usec - tvalBefore.tv_usec) / CLOCKS_PER_SEC;

printf("%f + %f = %f \n", timer, calc, (timer+calc));
      printf("%d + %d = %d \n", newSecs, timerSec, (newSecs+timerSec));
      if(newSecs<1000000)
         timer = timer + calc;
         //timerSec = (newSecs+timerSec);
   }
   else{
      int newSecs = tvalBefore.tv_sec - tvalAfter.tv_sec;
      if(newSecs<1000000)
         timerSec = (newSecs+timerSec);
...
void sortProcessesSJF(){
   struct PCB temp;
   temp.theJobs = malloc(5000 * sizeof(struct jobInfo));
   ...
   while(counter < numOfProcesses){
      for(inner = 0; inner < numOfProcesses; inner++){
         if (theProcesses[counter].numOfJobs > theProcesses[counter+1].numOfJobs){
            temp.numOfJobs = theProcesses[counter].numOfJobs;

            for (jobNdx = 0; jobNdx < theProcesses[counter].numOfJobs; ++jobNdx)
               {
                  temp.theJobs[jobNdx].component = theProcesses[counter].theJobs[jobNdx].component;
                  strcpy(temp.theJobs[jobNdx].operation, theProcesses[counter].theJobs[jobNdx].operation);
                  temp.theJobs[jobNdx].numOfCycles = theProcesses[counter].theJobs[jobNdx].numOfCycles;
                  temp.theJobs[jobNdx].processThisBelongsTo = theProcesses[counter].theJobs[jobNdx].processThisBelongsTo;
               }

            theProcesses[counter].numOfJobs = theProcesses[counter+1].numOfJobs;

            for (jobNdx = 0; jobNdx < theProcesses[counter+1].numOfJobs; ++jobNdx)
               {
                  theProcesses[counter].theJobs[jobNdx].component = theProcesses[counter+1].theJobs[jobNdx].component;
                  strcpy(theProcesses[counter].theJobs[jobNdx].operation, theProcesses[counter+1].theJobs[jobNdx].operation);
                  theProcesses[counter].theJobs[jobNdx].numOfCycles = theProcesses[counter+1].theJobs[jobNdx].numOfCycles;
                  theProcesses[counter].theJobs[jobNdx].processThisBelongsTo = theProcesses[counter+1].theJobs[jobNdx].processThisBelongsTo;
               }

            theProcesses[counter+1].numOfJobs = temp.numOfJobs;

            for (jobNdx = 0; jobNdx < temp.numOfJobs; ++jobNdx)
               {
                  theProcesses[counter+1].theJobs[jobNdx].component = temp.theJobs[jobNdx].component;
                  strcpy(theProcesses[counter+1].theJobs[jobNdx].operation, temp.theJobs[jobNdx].operation);
                  theProcesses[counter+1].theJobs[jobNdx].numOfCycles = temp.theJobs[jobNdx].numOfCycles;
                  theProcesses[counter+1].theJobs[jobNdx].processThisBelongsTo = temp.theJobs[jobNdx].processThisBelongsTo;
               }
