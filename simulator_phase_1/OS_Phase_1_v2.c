...
void startOperation(int location){
   /// check for start and end
	if ( !strcmp(theOperations[location].operation, "start" ) ){
            ...
            sprintf(finalData[dataNdx],"%06f - Simulator program starting\n", timerMicro);
            dataNdx++;
            ...
            addTime();
         	...
         	gettimeofday (&tvalBefore, NULL);
			   addTime();
            sprintf(finalData[dataNdx],"%06f - Simulator program ending\n", timerMicro);
            dataNdx++;
         }
}
...
void appOperation(int location){
	if ( !strcmp(theOperations[location].operation, "start" ) ){
			gettimeofday (&tvalBefore, NULL);
			addTime();
         sprintf(finalData[dataNdx],"%06f - OS: Preparing process %d\n", timerMicro, processCounter);
         dataNdx++;
         gettimeofday (&tvalAfter, NULL);
			addTime();
         sprintf(finalData[dataNdx],"%06f - OS: Starting process %d\n", timerMicro, processCounter);
         dataNdx++;
   }
   else{
       	gettimeofday (&tvalBefore, NULL);
			addTime();
         sprintf(finalData[dataNdx],"%06f - OS: Removing process %d\n",timerMicro, processCounter);
         dataNdx++;
         gettimeofday (&tvalAfter, NULL);
			addTime();
         // processCounter++; for next project
         }
}
...
void running (int location){
   ...
   /// save 
   sprintf(finalData[dataNdx],"%06f - Process %d : start processing action\n", timerMicro, processCounter);
   dataNdx++;
   ...
   /// save
   sprintf(finalData[dataNdx],"%06f - Process %d : end processing action \n\n", timerMicro, processCounter);
   dataNdx++;
}
...
void* inputOperation(void *arg){
   ...
   if ( !strcmp(theOperations[(*counter)].operation, "hard drive" ) ){
            ...
            sprintf(finalData[dataNdx],"%06f - Process %d : start hard drive input\n", timerMicro, processCounter);
            dataNdx++;
            ...
            sprintf(finalData[dataNdx],"%06f - Process %d : start keyboard input\n", timerMicro, processCounter);
            dataNdx++;
   ...
   if ( !strcmp(theOperations[(*counter)].operation, "hard drive" ) ){
            ...
            sprintf(finalData[dataNdx],"%06f - Process %d : end hard drive input\n\n", timerMicro, processCounter);
            dataNdx++;
         }
         else{
         	gettimeofday (&tvalAfter, NULL);
            ...
            sprintf(finalData[dataNdx],"%06f - Process %d : end keyboard input\n\n", timerMicro, processCounter);
            dataNdx++;
         }
...
void* outputOperation(void *arg){
   ...
   int waitTime;
   	...
   	if ( !strcmp(theOperations[(*counter)].operation, "hard drive" ) ){
            ...
            sprintf(finalData[dataNdx],"%06f - Process %d : start hard drive output\n",timerMicro, processCounter);
            dataNdx++;
            ...
            sprintf(finalData[dataNdx],"%06f - Process %d : start monitor output\n",timerMicro, processCounter);
            dataNdx++;
   ...
   if ( !strcmp(theOperations[(*counter)].operation, "hard drive" ) ){
            ...
            sprintf(finalData[dataNdx],"%06f - Process %d : end hard drive output\n\n",timerMicro, processCounter);
            dataNdx++;
         }
         else{
         	gettimeofday (&tvalAfter, NULL);
            ...
            sprintf(finalData[dataNdx],"%06f - Process %d : end monitor output\n\n",timerMicro, processCounter);
            dataNdx++;
         }
   ...
   /// check if the before or after is greater
	if (tvalAfter.tv_usec > tvalBefore.tv_usec){
      /// calculate the new seconds and microseconds by taking differences 
		...
		int newUsecs = (tvalAfter.tv_usec - tvalBefore.tv_usec);
      /// update the overall timer
		timerSec = timerSec + newSecs;
		timerMicro = timerMicro + newUsecs + (newSecs*1000000);
	}
