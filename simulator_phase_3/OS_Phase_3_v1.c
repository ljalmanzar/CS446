...
struct PCB{
   /// running type
   enum {
      enter, ready, running, waiting, done
   } state;
...
void addTime();
void output();
void sortProcessesSJF(int nrProcess);
...
int main(int argc, char* argv[]){
   ...
   if (!strcmp(theConfig.scheduling, "FIFO")){
      ...
      sortProcessesSJF(numOfProcesses);
...
*/
void* inputOperation(void *arg){
   struct jobInfo* theJob = (struct jobInfo*) arg;
   int waitTime;
...
void runProcess (int whichProcess){
   ...
   /// change state
   theProcesses[whichProcess].state = running;
   ...
   /// update state
   theProcesses[whichProcess].state = done;
}
...
void sortProcessesSJF(int nrProcess){
   ...
   while( !isSorted ){
      ...
      /// for all processes
      for(inner = 0; inner < nrProcess - 1; inner++){
